...
def topbar(request):
    if request.is_ajax and request.method == 'GET':
        ...
        for id in range(1,saved):
            if Moto.objects.get(id=id).num_sur_facture != None or  Moto.objects.get(id=id).num_BL != None:
                stock = stock -1
                # print("stock = ",stock)
        ...
        for op in operations:
            if not op.Prep_Full:
                ...
                counter = counter +1
        return JsonResponse({"notification":notification,'counter':counter,'numFactureLast':numFactureLast,'numBLlast':numBLlast,'idMotoLast':idMotoLast,'stock':stock},status=200)
